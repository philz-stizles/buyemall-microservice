...
// Services
import {
  uploadFile as cloudinaryUpload,
  removeFile as cloudinaryRemove,
} from '@devdezyn/buyemall-common'
...
export const uploadFile = async (
  ...
  const result = await cloudinaryUpload(req.body.image)
...
export const removeFile = async (
  ...
  await cloudinaryRemove(req.body.public_id, (err, result): Response => {
...
export const setProductRating = async (
  ...
  const existingRatingObject = existingProduct.ratings.find(
    ...
    rating => rating.postedBy.toString() === req.currentUser?.id.toString()
  )
  ...
  if (!existingRatingObject) {
    const ratingAdded = await Product.findByIdAndUpdate(
      existingProduct._id,
      {
        $push: { ratings: { star, postedBy: req.currentUser?.id } },
      },
      { new: true }
    ).exec()
